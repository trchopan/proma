...
test("buildTopicMergeContent normalizes optional-at canonical handles and deduplicates identity", () => {
  ...
  const result = buildTopicMergeContent({
    ...
    item: {
      category: "planning",
      source: "slack",
      summary: "Coordination updates with @ivy.example",
    ...
    mergeContent: {
      ...
      plan: [],
      timeline: ["2026-03-11 - Ownership clarified"],
      teamsIndividualsInvolved: ["ivy.example (slack:@ivy.example)"],
      references: [{ source: "slack", link: "https://example.com/thread" }],
      tags: ["coordination"],
    },
    target: {
  ...
  expect(result.proposedContent).toContain("- ivy.example (slack:ivy.example)");
  expect(result.proposedContent).not.toContain("(slack:@ivy.example)");
  expect(result.proposedContent).not.toContain("- (slack:ivy.example)");
});
